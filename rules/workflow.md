...
- **Testability + decomposition**: assess how hard the change is to verify and propose simplifications up front (sample/sandbox app, extracting logic to a no-device unit boundary) so a prototype is exercised fast before touching the real app; decompose a task too large for one plan. Detail: [[task-types]] § Test feasibility gate.
...
- **Runtime (L5) is out of scope by default** — no emulator/device driving. If a change genuinely cannot be verified any other way than on the running app, flag it and ask the user; do not boot an emulator silently.
