...
- Lower verification cost by extracting the changed logic to a **no-device unit / integration boundary** (pure function, API contract) rather than driving it through the UI. Device/emulator-dependent verification (instrumented UI, screenshot, E2E) is **out of scope** — see [[qa-and-testing]] § Verification pyramid.
...
| UI / design task | Figma / screenshots | L0–L1 | — | Visual match reviewed **manually** against the mockup — no automated UI / screenshot tests (device out of scope) |
| Performance optimization | Benchmark baseline — before/after numbers | L0–L2 where measurable without a device | Capture baseline before; measure delta after | Win must be a measurable delta; device-only macrobenchmarks (Macrobenchmark / Perfetto) are **out of scope for now** — flag if a claim needs them |
...
2. **No test coverage** → capture a **non-device** baseline before starting:
   ...
   - If the only meaningful baseline would require the running app on a device, prefer the test-coverage shortcut below (write tests first) — device capture is out of scope.
